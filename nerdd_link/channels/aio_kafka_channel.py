...
class AioKafkaChannel(Channel):
    ...
    def _get_connection_options(self) -> dict[str, Any]:
        username_provided = (
            self._broker_username is not None and self._broker_username.strip() != ""
        )
        password_provided = (
            self._broker_password is not None and self._broker_password.strip() != ""
        )

        if not username_provided and not password_provided:
            return {}

        if self._broker_username is None or self._broker_password is None:
            raise ValueError(
        ...
        return {
            ...
            "sasl_plain_username": self._broker_username,
            "sasl_plain_password": self._broker_password,
        }
